...
import { initVueInfo,
  ...
  resampleAndUpdate,
  createChart,
  createBrushChart, updateBrushChartLines,
  zoomIng,
  zoomEnd,
  resetZoom,
  handleMouseLeave,
...
export default {
  ...
  mounted() {
    this.generateChartList();   // 產生曲線圖列表資料
  ...
  methods: {
    ...
    updateChartData() {
      ...
      this.Setup.animateChartInterval = setInterval(() => {
        this.chartSeriesData.forEach(series => {
          var newData = {
            time: new Date(),
            value: Math.floor(Math.random() * 1000),
          }
          series.originalValues.push(newData);
          series.originalValues.shift();
        });
        this.updateOriginalScalesDomain();
        // 觸發重新採樣和繪圖流程
        this.resampleAndUpdate();
