...
var initVueInfo = function(info) {
  ...
  setTimeout(() => {
    ...
    that.initViewBox = { x: 0, y: 0, width: canvasRect["width"], height: canvasRect["height"] }
    that.svgViewBox = { x: 0, y: 0, width: canvasRect["width"], height: canvasRect["height"] };
...
var onMouseMove = function(event) {
  const canvasRect = that.svgViewBox;
  const canvasWidth = canvasRect.width; // 畫布寬度
  const canvasHeight = canvasRect.height; // 畫布高度
...
var onMouseUp = function(event) {
  ...
  if (drag_list.length != 0) {
    var checkMove = false
    var moveThreshold = 10; // 定義移動閾值
    for (let i in cpy_selectitem) {
      // 判斷是否有移動到x、y
      const diffX = drag_list[i]['n_x'] - cpy_selectitem[i]['n_x'];
      const diffY = drag_list[i]['n_y'] - cpy_selectitem[i]['n_y'];
...
var handleResizeWindow = function() {
  const canvasRect = getCanvasRect()
  if (canvasRect.width > that.svgViewBox.width || canvasRect.height > that.svgViewBox.height && that.Setup.fullscreenState == false) {
    that.initViewBox = { x: 0, y: 0, width: canvasRect.width, height: canvasRect.height };
    that.svgViewBox = { x: 0, y: 0, width: canvasRect.width, height: canvasRect.height };
...
var onResize = function(event) {
  ...
  // 獲取 SVG 邊界
  const canvasRect = getCanvasRect();
  const canvasWidth = that.svgViewBox?.width || canvasRect.width;
  const canvasHeight = that.svgViewBox?.height || canvasRect.height;
...
function getCanvasPoint(event) {
  const canvasElement = that.$refs.canvasCard?.$el
  const canvasRect = canvasElement?.getBoundingClientRect();
  const scrollLeft = canvasElement?.scrollLeft || 0;
  const scrollTop = canvasElement?.scrollTop || 0;
  if (!canvasRect) return { x: 0, y: 0 };
  const mouseXInContainer = event.clientX - canvasRect.left + scrollLeft;
  const mouseYInContainer = event.clientY - canvasRect.top + scrollTop;
  const pointX = (mouseXInContainer - that.svgOffset['x']) / that.svgOffset['scale'];
  const pointY = (mouseYInContainer - that.svgOffset['y']) / that.svgOffset['scale'];
