...
var moveItemsByOverlap = function(nodes, itemsToMove, direction) {
  var move = [...itemsToMove]
  if (!nodes || !move.length || (direction !== 'up' && direction !== 'down')) return null;
  const { x: offsetX, y: offsetY, scale: offsetScale } = this.svgOffset;
  // 計算群組的整體邊界框（最小包含矩形）
...
var setItemPosition = function(that, position) {
  ...
  // const w = that.svgViewBox['width'];
  // const h = that.svgViewBox['height'];
  // let target_cx, target_cy;
  // switch (position) {
  //   case "top-left":      target_cx = 0;      target_cy = 0;      break;
  //   case "top-center":    target_cx = w / 2;  target_cy = 0;      break;
  //   case "top-right":     target_cx = w;      target_cy = 0;      break;
  //   case "middle-left":   target_cx = 0;      target_cy = h / 2;  break;
  //   case "center":        target_cx = w / 2;  target_cy = h / 2;  break;
  //   case "middle-right":  target_cx = w;      target_cy = h / 2;  break;
  //   case "bottom-left":   target_cx = 0;      target_cy = h;      break;
  //   case "bottom-center": target_cx = w / 2;  target_cy = h;      break;
  //   case "bottom-right":  target_cx = w;      target_cy = h;      break;
  // }
  var filter_lock = [...that.selectcomponent_list].filter(item => !item.lock)
...
//   let svgDimension = that.svgViewBox[dimensionAxis]; // SVG寬度
