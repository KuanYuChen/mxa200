  ...
  <v-main style="background-color: rgb(233, 233, 233)">
    <div style="width: 100%;">
      <v-card style="border: 1px solid #dbd7d7; border-radius: 0px;">
        <!-- ========================================================== 畫布內容 ========================================================== -->
          <v-layout>
            <v-container ref="container" fluid class="fill-height pa-0 d-flex position-relative">
              <!-- 左側區塊 (圖片) -->
              <div ref="leftPane" class="left-pane" :style="{ width: leftWidth + '%' }">
                <!-- 修改頁面Header Bar -->
              ...
              <!-- 垂直分隔線 (左右拖曳) -->
              <div class="vertical-divider" :style="{ left: `calc(${leftWidth}% - 2.5px)` }" @mousedown.prevent="startDragVertical"></div>

              <!-- 右側區塊 -->
              <div v-show="leftWidth < 100" ref="rightBar" class="right-bar d-flex flex-column" :style="{ width: rightWidth + '%' }">
                <!-- 上區塊 -->
                <div ref="topPane" class="top-pane" :style="{ height: topHeight + '%' }">
                  <v-card variant="text">
                    <div class="emap-menu">
                ...
                <!-- 水平分隔線 (上下拖曳) -->
                <div class="horizontal-divider" @mousedown.prevent="startDragHorizontal"></div>

                <!-- 下區塊 -->
                <div ref="bottomPane" class="bottom-pane" :style="{ height: bottomHeight + '%' }">
                  <v-card variant="text" class="emap-menu">
                    <div style="display: flex; align-items: center; justify-content: center; margin: 10px 0px; gap: 10px;">
                      <div v-for="(item, index) in operate_menu_list" :key="index" style="display: flex; align-items: center; margin: 10px 0px;">
                        <v-tooltip :text="item.title" location="bottom">
                          <template v-slot:activator="{ props }">
                            <v-icon v-bind="props" @click="item.func()" :icon="item.icon" />
                          </template>
                        </v-tooltip>
                      </div>
                    </div>
...
<script>
  ...
  import { initVueInfo, handleBackgroundMouseDown, handleItemsMouseDown, onMouseMove, onMouseUp, handleItemsResizeDown, onResize, onResizeMouseUp, 
  ...
  export default {
    ...
    data() {
      return {
        ...
        minLeftWidth: 0.01, // 左側區塊最小寬度
        maxLeftWidth: 99.9, // 左側區塊最大寬度

        // 上下分隔
        topHeight: 50,
        draggingHorizontal: false,
        minTopHeight: 10, // 上方區塊最小高度
        maxTopHeight: 90, // 上方區塊最大高度
        
        // [優化] 新增 dragContext 用於暫存拖曳時的狀態
        dragContext: {}, 
        ...
        svgViewBox: { x: 0, y: 0, width: 2560, height: 1080 }, // SVG View Box
        ...
        operate_menu_list: [
          { title: '最上層', icon: 'mdi-arrange-bring-to-front', func: () => this.componentLevel('front') },
          { title: '最下層', icon: 'mdi-arrange-send-to-back' , func: () => this.componentLevel('back')  },
          { title: '上一層', icon: 'mdi-arrange-bring-forward' , func: () => this.componentOneLevel('up') },
          { title: '下一層', icon: 'mdi-arrange-send-backward' , func: () => this.componentOneLevel('down') },
          { title: '水平均分', icon: 'mdi-align-horizontal-distribute', func: () => this.distributedAlignment(this, 'horizontal') },
          { title: '垂直均分', icon: 'mdi-align-vertical-distribute'   , func: () => this.distributedAlignment(this, 'vertical')   },
          { title: '水平置中', icon: 'mdi-align-horizontal-center'     , func: () => this.centerComponents(this, 'horizontal')     },
          { title: '垂直置中', icon: 'mdi-align-vertical-center'       , func: () => this.centerComponents(this, 'vertical')       },
          { title: '靠上對齊', icon: 'mdi-align-vertical-top'          , func: () => this.setItemPosition(this, 'top')             },
          { title: '靠下對齊', icon: 'mdi-align-vertical-bottom'       , func: () => this.setItemPosition(this, 'bottom')          },
          { title: '靠左對齊', icon: 'mdi-align-horizontal-left'       , func: () => this.setItemPosition(this, 'left')            },
          { title: '靠右對齊', icon: 'mdi-align-horizontal-right'      , func: () => this.setItemPosition(this, 'right')           },
        ]
      };
    },
    watch: {
    ...
    beforeUnmount() {
      ...
      this.stopDragVertical();
      this.stopDragHorizontal();
    },
    mounted() {
    ...
    computed: {
      ...
      rightWidth() { return 100 - this.leftWidth; },
      bottomHeight() { return 100 - this.topHeight; },
      // 內層變換容器CSS
    ...
    methods: {
      ...
      startDragVertical(e) {
        ...
        const leftPaneEl = this.$refs.leftPane;
        const rightBarEl = this.$refs.rightBar;

        if (!containerEl || !leftPaneEl || !rightBarEl) return;
        ...
        document.addEventListener("mousemove", this.onDragVertical);
        document.addEventListener("mouseup", this.stopDragVertical);
      },

      onDragVertical(e) {
        if (!this.draggingVertical) return;

        if (!this.dragContext.ticking) {
          this.dragContext.ticking = true;
          window.requestAnimationFrame(() => {
            const { containerRect, leftPaneEl, rightBarEl } = this.dragContext;
            
            const offsetX = e.clientX - containerRect.left;
            let newLeftWidth = (offsetX / containerRect.width) * 100;
            
            newLeftWidth = Math.max(this.minLeftWidth, Math.min(newLeftWidth, this.maxLeftWidth));
            
            leftPaneEl.style.width = `${newLeftWidth}%`;
            rightBarEl.style.width = `${100 - newLeftWidth}%`;
            
            this.dragContext.finalWidth = newLeftWidth; 
            this.leftWidth = this.dragContext.finalWidth;

            this.dragContext.ticking = false;
          });
        }
      },
      stopDragVertical() {
        ...
        document.removeEventListener("mousemove", this.onDragVertical);
        document.removeEventListener("mouseup", this.stopDragVertical);
        this.dragContext = {};
      },
      ...
      startDragHorizontal() {
        ...
        const rightBarEl = this.$refs.rightBar;
        const topPaneEl = this.$refs.topPane;
        const bottomPaneEl = this.$refs.bottomPane;

        if (!rightBarEl || !topPaneEl || !bottomPaneEl) return;
        
        this.dragContext = {
          rightBarEl,
          topPaneEl,
          bottomPaneEl,
          rightBarRect: rightBarEl.getBoundingClientRect(),
          ticking: false,
        };

        document.addEventListener("mousemove", this.onDragHorizontal);
        document.addEventListener("mouseup", this.stopDragHorizontal);
      },
      ...
      onDragHorizontal(e) {
        ...
        if (!this.dragContext.ticking) {
          this.dragContext.ticking = true;
          window.requestAnimationFrame(() => {
            const { rightBarRect, topPaneEl, bottomPaneEl } = this.dragContext;
            
            const offsetY = e.clientY - rightBarRect.top;
            let newTopHeight = (offsetY / rightBarRect.height) * 100;

            newTopHeight = Math.max(this.minTopHeight, Math.min(newTopHeight, this.maxTopHeight));
            
            topPaneEl.style.height = `${newTopHeight}%`;
            bottomPaneEl.style.height = `${100 - newTopHeight}%`;

            this.dragContext.finalHeight = newTopHeight;
      ...
      stopDragHorizontal() {
        ...
        if (this.dragContext.finalHeight !== undefined) {
          this.topHeight = this.dragContext.finalHeight;
        }
        
        document.removeEventListener("mousemove", this.onDragHorizontal);
        document.removeEventListener("mouseup", this.stopDragHorizontal);
        this.dragContext = {};
      },
      // =========================================================================================
      ...
      emapLimitTest() {
        for (let i=1; i <= 125; i++) {
          ...
          var pictureinfo = {
            "borderColor": "#000000", "color": "#000000", "fontSize": 20, "func": "", "group": "All", "height": 200, "id": Math.floor(Math.random() * 100000000),
            "image": "1920_工作區域 1.png", "lock": false, "n_x": test_x, "n_y": test_y, "point": "", "showBorder": true, "showComponent": true,
            "title": "image_Default", "type": "image", "width": 200
          }
          var iconInfo = {
            "borderColor": "#000000", "color": "#000000", "fontSize": 20, "func": "", "group": "All", "height": 50, "icon": "mdi-account",
            "id": Math.floor(Math.random() * 100000000), "lock": false, "n_x": test_x, "n_y": test_y, "point": "", "showBorder": true,
            "showComponent": true, "title": "icon_Default", "type": "icon", "width": 50
          }
          var textinfo = {
            "borderColor": "#000000", "color": "#4BD3A7", "fontSize": 20, "func": "", "group": "All", "height": 100, "id": Math.floor(Math.random() * 100000000),
            "lock": false, "n_x": test_x, "n_y": test_y, "point": "", "showBorder": false, "showComponent": true, "title": "text_Default", "type": "text", "width": 200
          }
      ...
      emapComponentType(item) {
        const typeMap = {
          button: "ButtonBox", input: "InputBox", timer: "TimerBox", marquee: "MarqueeBox", image: "ImageBox", icon: "IconBox", linechart: "LinechartBox",
          devtable: "Devtable", monitor: "Monitor", website: "Website", polyline: "Polyline",
        };
      ...
      emapComponentProps(item, componentType) {
        return {
          componentChange: this.componentChange, item, 'dragitem': this.selectcomponent_list,
          blurEditText: () => this.blurEditText(item), changeEmap: (type) => this.changeEmap(type)
        };
      },
      // 組件CSS樣式
      getItemStyle(item, index) {
        var cssStyle = {
          position: 'absolute', left: `${item.n_x}px`, top: `${item.n_y}px`, width: `${item.width}px`, height: `${item.height}px`, zIndex: index+100,
        ...
        switch (item.type) {
          case "button": cssStyle['borderRadius'] = `${item.btnRadius}px`; break;
          case "devtable": cssStyle['border-radius'] = '5px'; cssStyle['user-select'] = 'none'; break;
          case "input": case "timer": case "marquee":
            cssStyle['border-radius'] = `${item.textRadius}px`; cssStyle['background'] = item.bgColor; cssStyle['overflow'] = 'hidden'; break;
          case "Group": delete cssStyle["border"]; break;
        }
        return cssStyle;
      },
      // 設定群組框顏色 (隨機)
      ...
      getGroupChildStyle(node) {
        ...
        let relativeX = 0; let relativeY = 0;
        if (parentGroup) { relativeX = node.n_x - parentGroup.n_x; relativeY = node.n_y - parentGroup.n_y; } 
        else { relativeX = node.n_x; relativeY = node.n_y; }
        var cssStyle = { 
          position: 'absolute', left: `${relativeX}px`, top: `${relativeY}px`, width: `${node.width}px`, height: `${node.height}px`, zIndex: 2, outlineOffset: '1px',
          border: `2px solid ${this.selectcomponent_list.includes(node) && !node.lock ? "blue" : node.showBorder ? node.borderColor : "#00000000"}`,
          'will-change':'transform', outlineOffset: '2px' 
        }
        switch (node.type) {
          case "button": cssStyle['border-radius'] = `${node.btnRadius}px`; break;
          case "devtable": cssStyle['border-radius'] = '5px'; cssStyle['user-select'] = 'none'; break;
          case "input": case "timer": case "marquee":
            cssStyle['border-radius'] = `${node.textRadius}px`; cssStyle['background'] = node.bgColor; cssStyle['overflow'] = 'hidden'; break;
        }
        return cssStyle;
      },
      // Resize框CSS參數
      getResizeStyle(item, corner) {
        ...
        const size = 10; const offset = size / 2;
        const positions = {
        ...
        var handleStyle = { position: 'absolute', width: `${size}px`, height: `${size}px`, backgroundColor: '#dodgerblue', zIndex: 9999, ...positions[corner] };
        return handleStyle
      },
      // Dbclick組件選擇
      toggleEditMode(item) {
        ...
        const isCurrentlyEditing = itemToEdit.edit;
        ...
        if (this.selectcomponent_list.length === 1 && this.selectcomponent_list[0].id === itemToEdit.id) {
          itemToEdit.edit = !isCurrentlyEditing;
        } else {
          this.selectcomponent_list = [itemToEdit];
      ...
      getEmapData(state = 'init') {
        ...
        useSetup().getPageInfo({idx: this.paginationID}).then((res)=> {
          ...
          this.emapInfo = {
            id: this.paginationID, content: { image: "", background: "#FFFFFF", imageopacity: 1, nodes: [] },
            headerbar: { title: "Default-Title", background: "#569981", color: "#FFFFFF", btn: [] }, navbar: { background: "#B5BDBC", btn: [] },
          }
        }).finally(()=> {
          this.initEmapData()
          if (state == 'init') {
      ...
      initEmapData() {
        ...
        this.svgBG = { image: initEmapInfo["content"]["image"] || "", color: initEmapInfo["content"]["background"] || "#FFFFFF", imageopacity: initEmapInfo["content"]["imageopacity"] || 1 }
      ...
      checkToSave(showdialog) {
        var remove_key = ['edit', 'imagefile', 'timenow', 'hover', 'max_val', 'max_x', 'min_val', 'min_x', 'dragOrigin', 'idname']
        const filterNode = (node) => {
          const filtered = Object.keys(node).reduce((acc, key) => {
            if (!remove_key.includes(key)) acc[key] = node[key];
            return acc;
          }, {});
          if (node.type === 'Group' && Array.isArray(node.nodes)) filtered.nodes = node.nodes.map(child => filterNode(child));
          return filtered;
        };
        const filterNodes = this.svg_nodes.map(node => filterNode(node));
        this.emapInfo = {
          headerbar: this.emap_headerbar, navbar: this.emap_navbar,
          content: { nodes: filterNodes, image: this.svgBG["image"], background: this.svgBG["color"], imageopacity: this.svgBG["imageopacity"], }
        }
      ...
      removeComponent(item) {
        ...
        for (let i in this.svg_nodes) {
          ...
          if (node.id == item.id) { this.removeTargetFromEmap(this.svg_nodes, i); return; }
        }
      },
      ...
      // =================================== 操作選單 ===================================
      componentLevel(type) {
        if (this.selectcomponent_list.length === 0) { this.addSnackbar(`請先選擇要在最${type == 'front' ? '上' : '下'}層的組件`, "red"); return; }
        if (this.selectcomponent_list.some(item => item.lock)) { this.addSnackbar(`選取的組件中含有鎖定組件，無法${type == 'front' ? '上' : '下'}移`, "red"); return; }
      ...
      componentOneLevel(type) {
        ...
        if (this.selectcomponent_list.some(item => item.lock)) { this.addSnackbar(`選取的組件中含有鎖定組件，無法${type == 'up' ? '上' : '下'}移`, "red"); return; }
        const newNodes = this.moveItemsByOverlap(this, this.svg_nodes, this.selectcomponent_list, type);
        if (newNodes) { this.svg_nodes = newNodes; this.changeEmap(`組件${type == 'up' ? '上' : '下'}移一層`); } 
        else this.addSnackbar(`組件已在最${type == 'up' ? '上' : '下'}層`);
      },
      // =================================== 群組操作 ===================================
      addComponentGroup(items) {
        const groupList = items.flatMap(item => {
          if (item.type === "Group") { this.svg_nodes = this.svg_nodes.filter(node => node.id !== item.id); return item.nodes; } 
          else { return [item]; }
        });
        const minX = Math.min(...groupList.map(i => i.n_x)), minY = Math.min(...groupList.map(i => i.n_y));
        const maxX = Math.max(...groupList.map(i => i.n_x + i.width)), maxY = Math.max(...groupList.map(i => i.n_y + i.height));
        const newGroup = {
          id: crypto.randomUUID(), type: "Group", n_x: minX, n_y: minY, width: maxX - minX, height: maxY - minY,
          lock: false, showComponent: true, nodes: groupList,
        };
        const groupedIds = new Set(groupList.map(i => i.id));
        this.svg_nodes = this.svg_nodes.filter(node => !groupedIds.has(node.id));
        this.svg_nodes.push(newGroup);
        this.regenerateAllIdnames();
        this.changeEmap("新增群組");
      },
      cancelGroup(type, target) {
        if (type === 'Right Click') {
          const groupItem = target.item[0];
          groupItem.nodes.forEach(node => { this.svg_nodes.push(node); });
          this.svg_nodes = this.svg_nodes.filter(node => node.id !== groupItem.id);
          ...
          for (let i = 0; i < this.svg_nodes.length; i++) {
            ...
            if (component.type === "Group") {
              ...
              if (index !== -1) {
                this.svg_nodes.push(target); component.nodes.splice(index, 1);
                if (component.nodes.length === 1) { this.svg_nodes.push(component.nodes[0]); this.svg_nodes.splice(i, 1); }
                this.changeEmap("取消群組"); return;
              }
            }
          }
        }
      ...
      uploadEmapJSON(event) {
        var input = event.target; this.emapImport = null;
      ...
      importEmapData() {
        ...
        reader.onload = (event) => {
          var data = event.target.result; var parseData = JSON.parse(data); parseData["id"] = this.paginationID;
          ...
          useSetup().setPageInfo(Query).finally(()=>{ 
            ...
            this.emapImport = null; this.getEmapData();
          })
        };
        reader.readAsText(this.emapImport);
      },
      exportEmapData() {
        const jsonData = JSON.stringify(this.emapInfo, null, 2);
        ...
        a.download = `export(${formattedTS}).json`; document.body.appendChild(a);
        a.click(); URL.revokeObjectURL(url); document.body.removeChild(a);
      },
      // =================================== 組件設定(Dialog) ===================================
      openComponent(state, item) {
        if (state == "EDIT" && item.lock) return;
        this.config["dialog"] = true; this.config["state"] = state;
        if (state == "EDIT") { this.config["info"] = JSON.parse(JSON.stringify(item)); }
        else {
          this.config['info'] = this.componentInitTemplate();
        } 
      },
      uploadImg(event) {
        var input = event.target; this.imgImport = null;
        if (input.files && input.files[0]) this.imgImport = input.files[0]
      },
      addComponent() {
        var items = this.config['info']; var ID =  crypto.randomUUID();
        const info = { id: ID, lock: false, order: this.svg_nodes.length + 1, ...items };
        this.setFormate(info);
        if (items['type'] == ""){ useSetup().showAlertDialog({ icon: "error", title: "請選擇組件類型" }); return; }
        if (items['type'] == 'image') {
          if (!this.imgImport) { useSetup().showAlertDialog({ icon: "error", title: "沒有選擇圖片" }); return; }
          info['image'] = this.imgImport["name"]; let formData = new FormData(); formData.append("file", this.imgImport);
          useSetup().uploadImage({ file: formData }).finally(()=> { ADD(this, info); return; })
        } else ADD(this, info);
        function ADD(that, data) { that.svg_nodes.push(data); that.checkToSave(false); that.closeComponent(); }
      },
      editComponent() {
        var items = this.config['info']; this.setFormate(items);
        if (items['type'] == "Group") { for (let i in items['nodes']) { var nodes = items['nodes'][i]; this.setFormate(nodes); } }
        for (let i in this.svg_nodes) {
          var node = this.svg_nodes[i]
          if (node.type == "Group") {
            for (let j in node.nodes) {
              if (node.nodes[j].id == items.id) { node.nodes[j] = items; this.checkToSave(false); this.closeComponent(); return; }
            }
          }
          if (node.id == items.id) {
            Object.keys(node).forEach(key => { if (!(key in items)) delete node[key] }); Object.assign(node, items);
            if (items["type"] == "image") {
              if (this.imgImport == null && items['image'] == undefined) { useSetup().showAlertDialog({ icon: "error", title: "沒有選擇圖片" }); return }
              let formData = new FormData(); formData.append("file", this.imgImport);
              useSetup().uploadImage({ file: formData }).finally(()=> {
                items['image'] = this.imgImport ? this.imgImport["name"] : items["image"]; node['image'] = items['image']; EDIT(this); return;
              })
            } else EDIT(this);
          }
        }
        function EDIT(that) { that.checkToSave(false); that.closeComponent(); }
      },
      closeComponent() {
      ...
      // =================================== 背景資訊設定(Dialog) ===================================
      openBackground(item) { this.bgItem["dialog"] = true; this.bgItem["info"] = item; },
      editImage(event) {
        const file = event.target.files[0];
        if (file) {
          let formData = new FormData(); formData.append("file", file);
          useSetup().uploadImage({ file: formData }).finally(()=> {
            this.svgBG["image"] = file["name"]; this.emapInfo["content"]["image"] = file["name"];
      ...
      removeBg() {
        this.svgBG["image"] = "";
        this.changeEmap("Remove Background Image", false).then(()=>{ useSetup().showAlertDialog({ icon: "success", title: "刪除底圖" }) })
      },
      editBackground() {
        this.svgBG["color"] = this.bgItem["info"]["background"]; this.svgBG["imageopacity"] = this.bgItem["info"]["imageopacity"]; this.svgBG["image"] = this.bgItem["info"]["image"];
        this.changeEmap("修改背景資訊").then(()=>{ this.closeBackground(); })
      },
      closeBackground() { this.bgItem["dialog"] = false; },
      // =================================== Headerbar設定(Dialog) ===================================
      openHeaderbar() { this.headerbarItem['dialog'] = true; this.headerbarItem['info'] = JSON.parse(JSON.stringify(this.emap_headerbar)); },
      editHeaderbar() {
        if (this.headerbarItem['info']['title'] == "") this.headerbarItem['info']['title'] = "Default-Title"
        this.emap_headerbar = this.headerbarItem['info']; this.emapInfo["headerbar"] = this.emap_headerbar;
        this.changeEmap("Headerbar", false).then(()=>{ this.closeHeaderbar() });
      },
      closeHeaderbar() { this.headerbarItem['dialog'] = false; },
      // =================================== Navbar設定(Dialog) ===================================
      openNavbar() { this.navbarItem['dialog'] = true; this.navbarItem['info'] = JSON.parse(JSON.stringify(this.emap_navbar)); },
      editNavbar() {
        this.emap_navbar = this.navbarItem['info']; this.emapInfo["navbar"] = this.emap_navbar;
        this.changeEmap("Navbar", false).then(()=>{ this.closeNavbar() });
      },
      closeNavbar() { this.navbarItem['dialog'] = false; },
      // =================================== Function ===================================
      setFormate(item) {
        var int_list = ['width', 'height', 'n_x', 'n_y']
        for (let i in int_list) { if (item[int_list[i]]) item[int_list[i]] = parseInt(item[int_list[i]]) }
      },
      removeTargetFromEmap(nodeData, idx) { nodeData.splice(idx, 1); this.changeEmap("移除組件"); },
      // 更新Emap資料 (未儲存資料)
