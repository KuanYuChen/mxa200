import { getCanvasRect, getVueInfo } from "./emap";
// ======================================= 鍵盤應用 =======================================
...
var handleKeydown = function(ev) {
  ...
  if (!that.componentChange) {
    ...
    // Enter 
    if (ev.key === "Enter") {
      if (that.config['dialog']) {
        if (that.config['state'] == "ADD") that.addComponent();
        else if (that.config['state'] == "EDIT") that.editComponent();
      }
    ...
    if (that.config['dialog'] || that.bgItem["dialog"] || that.headerbarItem["dialog"] || that.navbarItem["dialog"]) return;
...
var closeDialogWindow = function(that) {
  ...
  if (parts.length == 3 && parts[1] == "edit") {
    if (that.config['dialog']) that.closeComponent();
...
// **Ctrl + X - 剪下**
var cutItems = function(type) {
  var that = getVueInfo()
  pastedTimes = 0
  copy_items = []
  copy_items = that.selectcomponent_list.filter(item => !item.lock).map(item => ({ ...item })); // 深拷貝 (過濾鎖定組件)
  console.log("剪下:", copy_items);
  // 刪除剪下的組件
  const idsToRemove = new Set(that.selectcomponent_list.map(item => item.id))
  const filtered = that.svg_nodes.filter(item => !idsToRemove.has(item.id))
  that.svg_nodes = filtered
  that.addSnackbar(type == 'ctrl_x' ? "鍵盤動作 -> Ctrl + X - 剪下組件" : "剪下組件"); // 區分鍵盤、右鍵Menu
  saveHistory();
}
...
var copyItems = function(type) {
  ...
  that.addSnackbar(type == 'ctrl_c' ? "鍵盤動作 -> Ctrl + C - 複製組件" : "複製組件"); // 區分鍵盤、右鍵Menu
}
...
var pasteItems = function(type) {
  ...
  var that = getVueInfo()
  pastedTimes ++
  const svgBox = getCanvasRect();
  const svgWidth = svgBox.width;    // SVG 寬度
  const svgHeight = svgBox.height;  // SVG 高度
  const pastedItemsData = JSON.parse(JSON.stringify(copy_items)); // 深拷貝複製的項目

  const finalPastedItems = pastedItemsData.map((item, index) => {
    // --- 計算理想的偏移後位置 ---
    ...
    let newItem = {
      ...
      order: that.svg_nodes.length + index + 1,
      n_x: idealX, // 使用限制後的 X
      n_y: idealY, // 使用限制後的 Y
    };
    ...
    if (newItem.type === "Group" && Array.isArray(newItem.nodes)) {
      ...
      newItem.nodes = newItem.nodes.map((node, idx) => {
        // 計算子節點的理想新位置
        ...
        return {
          ...node,
          id: crypto.randomUUID(), // Math.floor(Math.random() * 10000000),
          order: that.svg_nodes.length + idx + 1, 
          n_x: idealNodeX, // 使用限制後的子節點 X
          n_y: idealNodeY, // 使用限制後的子節點 Y
        };
      });
      console.log("paste: ", newItem.nodes)
    }
    return newItem;
  });
...
var initHistory = function(state) {
  save_history = [JSON.stringify(state)];
  historyIDX = 0;
}
...
// ======================================== 設定組件類型 Function ========================================
var regenerateAllIdnames = function () {
  const that = getVueInfo();
  const typeCountMap = {};
  const deferredGroupMap = new Map(); // Map<index, groupNode>

  function assignIdname(node) {
    const type = node.type;
    if (!typeCountMap[type]) typeCountMap[type] = 1;
    else typeCountMap[type]++;
    node.idname = `${type}_${typeCountMap[type]}`;
  }

  // 第一輪：先處理非 Group 節點
  that.svg_nodes.forEach((node, index) => {
    if (node.type === 'Group' && Array.isArray(node.nodes)) {
      deferredGroupMap.set(index, node); // 記下 group 及其索引
    } else {
      assignIdname(node);
    }
  });

  // 第二輪：依照原順序處理 Group（包含內部子元件）
  deferredGroupMap.forEach((groupNode, index) => {
    groupNode.nodes.forEach(child => assignIdname(child));
    assignIdname(groupNode);
  });
};
...
export {
  handleKeydown, 
  cutItems, copyItems, pasteItems, deleteItems,
