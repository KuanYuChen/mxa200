<!-- src/components/NxPlayerFinalProxy.vue -->
<template>
  <v-main>
    <div class="nx-media-player-container">
      <h2 class="title">Nx Witness 播放器 (代理轉換模式)</h2>
...
export default {
  ...
  methods: {
    ...
    async fetchRecordedChunks() {
      ...
      try {
        const response = await fetch(
          `/media-proxy/${this.selectedCameraId}.webm?pos=${startTimeMs}&endPos=${endTimeMs}`,
          // `/rest-api-proxy/devices/${this.selectedCameraId}/recordedTime?pos=${startTimeMs}&endPos=${endTimeMs}`,
    ...
    async getPlayableVideoUrl(cameraId, timestampMs = null) {
      ...
      if (timestampMs) { apiUrl += `?pos=${timestampMs * 1000}`; }
