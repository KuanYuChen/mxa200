  ...
  methods: {
    ...
    startUpdatingData() {
      ...
      this.linechartInterval = setInterval(() => {
        if (this.chart_data.length === 0 || this.chart_data[0].values.length === 0) return;

        const lastTime = this.chart_data[0].values[this.chart_data[0].values.length - 1].time;
        const newTime = new Date(lastTime.getTime() + timeInterval);

        this.chart_data.forEach(series => {
          series.values.shift();
          series.values.push({ time: newTime, value: Math.floor(Math.random() * 90) + 10 });
        });
        
        this.xScale.domain([this.chart_data[0].values[0].time, newTime]);
        this.xAxis.transition().duration(1000 * 0.9).ease(d3.easeLinear)
            .call(d3.axisBottom(this.xScale).ticks(5).tickFormat(d3.timeFormat("%H:%M:%S")));
